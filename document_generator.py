...
def image_to_bytes(image, format="PNG"):
    """
    Fungsi Wajib untuk Bot: Mengubah gambar menjadi Bytes
    agar bisa dikirim langsung ke Telegram/Discord.
    """
...
def get_font(size, bold=False):
    ...
    system_fonts = ["arialbd.ttf", "Arial Bold"] if bold else ["arial.ttf", "Arial"]
...
def get_fonts_collection():
    return {
        "huge": get_font(60, bold=True),
        "title": get_font(48, bold=True),
        "heading": get_font(38, bold=True),
        "subheading": get_font(30, bold=True),
        "large": get_font(26, bold=False),
        "normal": get_font(22, bold=False),
        "small": get_font(18, bold=False),
        "tiny": get_font(14, bold=False)
    }
...
def generate_faculty_id(
    ...
    logo_size = 140
    logo_x, logo_y = 50, 30
    draw.ellipse([(logo_x, logo_y), (logo_x + logo_size, logo_y + logo_size)], fill="white", outline=accent_color, width=4)
    ...
    if len(school_name) > 25: school_font = get_font(45, bold=True)
    ...
    photo_y = header_h + 50
    border = 6
    draw.rectangle([(photo_x - border, photo_y - border), (photo_x + photo_w + border, photo_y + photo_h + border)], fill=primary_color)
...
def generate_pay_stub(teacher_name, teacher_email, school_name, emp_id, department):
    ...
    col2_x = W/2 + 30
    pay_date = datetime.now() - timedelta(days=random.randint(5, 25))
...
def generate_employment_letter(teacher_name, teacher_email, school_name, emp_id, department):
    ...
    lines = textwrap.wrap(text_body, width=70)
    for line in lines:
        if line == "": 
            current_y += 30
        else:
